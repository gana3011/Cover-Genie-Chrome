...
document.getElementById("greeting").innerText =
  `Welcome to CoverGenie 
${userName}` || "Welcome to CoverGenie";
  ...
  chrome.tabs.sendMessage(
    ...
    async (response) => {
      if (response && response.companyName) {
        ...
        alert(
          "Failed to extract job details. Please make sure you're on a job listing page."
        );
      }
    }
  );
...
document
  .querySelectorAll("#generateCoverLetter, #generateDm")
  .forEach((button) => {
...
Ensure the message is polite, professional, and clearly conveys interest in the role. Keep it short (2-3 sentences) and include a call to action, such as asking for a quick chat or more details about the job.`;
      ...
      coverLetterTextarea.innerText = responseText;
...
document.getElementById("copy").addEventListener("click", () => {
  ...
  navigator.clipboard
    .writeText(coverLetterTextarea.value)
